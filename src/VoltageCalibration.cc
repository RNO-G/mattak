    ...
    for (int i = 0; i < mattak::k::num_lab4_samples; i++)
    {
      ...
      for (unsigned j = 0; j < scanSize(); j++)
      {
        ...
        if (v >= fit_min && v <= fit_max)
        {
          ...
          {
            graph[ichan][i]->SetPoint(graph[ichan][i]->GetN(),adc,v);
            jmax = j;
          }
          last_adc = adc;
      ...
      double *data_adc = graph[ichan][i]->GetX();
      double *data_v = graph[ichan][i]->GetY();

      for(int i = 0; i < npoints; i++)
      {
        fit.AddPoint(&data_adc[i], data_v[i]);
      }
...
TGraph * mattak::VoltageCalibration::makeSampleGraph(int chan, int samp, bool resid) const
{
  ...
  double *data_adc = graph[chan][samp]->GetX();
  double *data_v = graph[chan][samp]->GetY();
